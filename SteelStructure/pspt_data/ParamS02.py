    ...
    def execute(self, obj):
        ...
        if key1==0:#アングル
            ...
            if Type=='s03':#s03
                ...
                c01=c00
                #try:
                c01.Placement=App.Placement(App.Vector(L,0,A+H1),App.Rotation(App.Vector(0,1,0),-90))
                #except:
                #    pass
                c1=c1.fuse(c01)
            #柱ベースプレート
            ...
            if BasePlate==True:
                ...
                angle_bp(self)
                c01=c00
                c01.Placement=App.Placement(App.Vector(L1,-(y-A)/2,-10),App.Rotation(App.Vector(0,1,0),-90))
                c1=c1.fuse(c01)
            ...
            if Type=='s03':#s03
                if BasePlate==True:
                    ...
                    c01=c00
                    #ry:
                    c01.Placement=App.Placement(App.Vector(L1,-(y-A)/2,H1-10),App.Rotation(App.Vector(0,1,0),-90))
                    #except:
                    #    pass
                    c1=c1.fuse(c01)
            ...
            if Type=='s03':
                channel2(self)
                c01=c00
                c01.Placement=App.Placement(App.Vector(L1+H0-t0,0,H0+H1),App.Rotation(App.Vector(0,1,0),-90))
                c1=c1.fuse(c01)

            if BasePlate==True:
                channel_bp(self)#ベースプレート柱
                c01=c00
                c01.Placement=App.Placement(App.Vector(-(x-H0)/2,-(y-B)/2,H0+H-t0),App.Rotation(App.Vector(0,0,1),0))
                c1=c1.fuse(c01)
                channel_bp(self)#ベースプレート梁　下
                c01=c00
                c01.Placement=App.Placement(App.Vector(L1+H0,-(y-B)/2,-(x-H0)/2),App.Rotation(App.Vector(0,1,0),-90))
                c1=c1.fuse(c01)

            if Type=='s03':
                if BasePlate==True:
                    channel_bp(self)#ベースプレート梁　上
                    c01=c00
                    c01.Placement=App.Placement(App.Vector(L1+H0,-(y-B)/2,-(x-H0)/2+H1),App.Rotation(App.Vector(0,1,0),-90))
                    c1=c1.fuse(c01)
            c1.Placement=App.Placement(App.Vector(-L1/2-H0,0,-H-H0),App.Rotation(App.Vector(0,0,1),0))  
            c1=c1.removeSplitter()  
            obj.Shape=c1          
